//SCCSID "@(#) samples/jms/interactive/helper/BaseOptions.java, jmscc.samples, k710, k710-007-151026  1.6.2.1 11/10/17 15:52:23"
/*
 * <N_OCO_COPYRIGHT>
...
abstract class BaseOptions {
	 ...
	 * Logger
	 */
	private static final Logger logger = LogManager.getLogger(BaseOptions.class);
	
  /**
   * Input messages for options.
   */
  protected static Hashtable inputMessage;

  /**
   * Help messages for options.
   */
  protected static Hashtable helpMessage;

  /**
   * Default values for options.
   */
  protected static Hashtable defaultValue;

  /**
   * Domain restriction for options. Domain restriction is a set of valid values for an option.
   */
  protected static Hashtable domainRestriction;

  /**
   * Validity checker methods for options. Assume at most one checker method per option.
   */
  protected static Hashtable isValid;

  /**
   * Map user input to some value.
   */
  protected static Hashtable nameToValue;
  ...
  public void Present(boolean mandatory) {
    ...
    try {
      ...
      // been presented
    	...
    	logger.error("Error: Unable to read input from standard input.");
    	logger.error(ioex);
    }
  ...
  public String Value() {
    ...
    if (nameToValue.containsKey(key)) {
      return ((Hashtable) nameToValue.get(key)).get(val).toString();
    }
    return val;
  } // end get_Value
  ...
  private String InputMessage() {
    return (String) inputMessage.get(key);
  }
  ...
  private String[] HelpMessage() {
    return (String[]) helpMessage.get(key);
  }
  ...
  private String[] DomainRestriction() {
    return (String[]) domainRestriction.get(key);
  }
  ...
  private boolean IsValid(String input) {
    return ((IsValidType) isValid.get(key)).Check(input);
  }
